...
export default function TaxMasterPage() {
  ...
  const handleExcel = () => {
    if (taxMaster.length === 0) {
      window.alert('Tax Master is empty.');
      return;
    }
...
function printSubTab(tab: SubTab, rows: DerivedRow[]): void {
  const tabLabels: Record<SubTab, string> = {
    brew:  'Brew & Fermentation',
    cond:  'Conditioning',
    total: 'Total Tax Page',
  };
  const headers = tab === 'brew' ? BREW_HEADERS : tab === 'cond' ? COND_HEADERS : TOTAL_HEADERS;
  const buildRow = tab === 'brew' ? brewRow : tab === 'cond' ? condRow : totalRow;
